...
%token NUM VAR IF ELSE FUNC MAIN INT FLOAT CHAR WHILE START END SWITCH CASE DEFAULT BREAK FOR PF SIN COS TAN LOG BIGMOD LOG10 FACT GCD LCM PRIME POWER PYRAMID
%nonassoc IFX
%nonassoc ELSE
%nonassoc SWITCH
%nonassoc CASE
...
program:
 ...
 MAIN ':' START cstatement END function
	 ;
...
statement:
  ...
  |	FUNC '('')'  { /*printf("function called");*/}
	| FOR '(' NUM '<' NUM ')' START statement END {
	                                int i;
	                                for(i=$3 ; i<$5 ; i++) {printf("value of the for loop: %d expression value: %d\n", i,$8);}
				               }
	| WHILE '('NUM')' START statement END {
		int temp =$3;
		while(temp--)
		{
...
function:
	| func function
	;
func: TYPE FUNC '('')' START statement END
			{
				/*printf("func func");*/
			}
			;
...
expression: NUM					{ $$ = $1; 	}
	...
	| FACT expression {int x=$2;int ans = 1;for(int i=1;i<=x;i++)ans*=i;
